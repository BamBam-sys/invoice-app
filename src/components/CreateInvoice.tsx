...
// const addDays = (numOfDays: number, currDate: Date) => {
//   const futureDate = currDate;
//   futureDate.setDate(futureDate.getDate() + numOfDays);
//   return futureDate;
// };

// const calculatePaymentDue = (numOfDays: number, currenDate: Date) => {
//   const futureDate = addDays(numOfDays, currenDate);
//   const paymentDue = formatDate(futureDate);
//   return paymentDue;
// };

const CreateInvoice = ({ panel, handleClick, setPanel }: props) => {
  // prevent scroll if panel is open
  useEffect(() => {
    !panel
  ...
  const handleCalendar = (value: Date) => {
    ...
    setInput((prev) => ({ ...prev, createdAt: date }));
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative flex justify-between w-full bg-white dark:bg-secondary-300  pr-[5.6rem] pl-[11.1rem] py-[3.1rem] rounded-br-[20px] rounded-tr-[20px] z-[5]">
        <Button text={'Save'} colors={'saveBtn'} />
        <div className="flex gap-x-[0.8rem] ">
          {' '}
          <Button text={'Save as Draft'} colors={'saveAsDraftBtn'} />
          <Button text={'Save & Send'} colors={'saveAndSendBtn'} />
        </div>
      </div>
    </div>
  );
